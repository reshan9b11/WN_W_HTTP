...
WifiDlOfdmaExample::Setup (void)
{
  ...
  m_sinkApps = SinkHelper.Install (m_Nodes);
...
WifiDlOfdmaExample::Run (void)
{
  ...
  for (uint32_t i = 0; i < m_staNodes.GetN (); i++)
    {
    	std::cout<<"\n rx start"<<m_rxStart[i]<<" "<<m_rxStop[i];
...
//  dest.SetTos (0xb8); //AC_VI
  ...
  {
   OnOffHelper client2 (socketType, InetSocketAddress (m_staInterfaces.GetAddress (m_currentSta), m_port));
  client2.SetAttribute ("OnTime", StringValue ("ns3::ExponentialRandomVariable[Mean=0.35]"));
  client2.SetAttribute ("OffTime", StringValue ("ns3::ExponentialRandomVariable[Mean=0.65]"));
  client2.SetAttribute ("DataRate", DataRateValue (DataRate (m_dataRate * 1e6)));
  client2.SetAttribute ("PacketSize", UintegerValue (m_payloadSize));

  InetSocketAddress dest (m_staInterfaces.GetAddress (m_currentSta), m_port);
  dest.SetTos (0xb8); //AC_VI
  client2.SetAttribute ("Remote", AddressValue (dest));
  std::cout<<"\n inside ONOFF helper";
  ...
  for (uint32_t i = 0; i < m_staNodes.GetN (); i=i+1)
    {
      ...
      clientApp->SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0]"));
    	...
    	if(i%3==1)
    	{
    		std::cout<<"\n BULK"<<i;
      Ptr<Application> clientApp = m_clientApps.Get (i);
      clientApp->SetAttribute ("MaxBytes", UintegerValue (0));
